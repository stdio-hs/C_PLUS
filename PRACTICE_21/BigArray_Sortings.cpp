...
// 하이브리드정렬 (선택정렬+퀵정렬)
void hybridQuickSelectionSort(int* bigArray, int size)  // 동적메모리 할당 받은 사이즈로 처음과 끝 구하는 함수.
{
	_hybridQuickSelectionSort(bigArray, 0, size - 1);
}
void _hybridQuickSelectionSort(int* bigArray, int left, int right)
{
	...
	// < 중간 설명 >
	// 배열 크기가 50이하 (컴마다 다름) 일 경우, 선택정렬이 더욱 빠른 예외상황을 가지고 있다.
	// 따라서 배열 50이하는 선택정렬 + 그 이상은 퀵정렬 = 하이브리드 함수를 만든다.
	// 먼저 선택정렬을 50만큼 하고 퀵정렬을 돌리는 것은 의미가 없다.
	// 따라서 퀵정렬을 사용하여 배열 50개를 남겨둘때까지 정렬을 시켜준다.
	// part = right - left 로 part가 50개 안으로 들어왔을 때, 퀵이 아닌, 선택정렬로 정렬을 마무리해준다. 

	// 1. 퀵정렬 구간
	if (left <= right) // 좌측 끝값 <= 우측 끝값 일 경우.
	{
		int pivot = _partition(bigArray, left, right); // 둘로 나누어서

		if (left < (pivot - 1)) // 피봇이 좌측 끝이 아닐 경우.
		{
			_hybridQuickSelectionSort(bigArray, left, pivot - 1); // 하나를 줄이고 우측 끝으로 보낸 후, 재정렬시킨다.
		}

		if ((pivot + 1) < right) // 피봇이 우측 끝이 아닐 경우.
		{
			_hybridQuickSelectionSort(bigArray, pivot + 1, right); // 하나를 더하고 좌측 끝으로 보낸 후, 재정렬시킨다.
		}
	}
}
void hy_selectionSort(int* array, int part, int left, int right)
{
	int index_min;

	for (int a = left; a < right - 1; a++) // array[0] 부터 array[size-1] 까지 하나하나 비교 + 대입.
	{
		index_min = a; // 순차적으로 선택정렬을 할 때, 가장 왼쪽인 부분을 마킹해놓는다.

		for (int b = a + 1; b < part; b++) // < 마킹한 곳 다음 것.> ~ < size(끝까지) > 하나하나 비교한다. 
		{
			if (array[b] < array[index_min]) // 비교했을 때, ( array[비교대상] < array[마킹한 곳] ) 일 경우
			{
				index_min = b; // 해당 비교대상(b)을 마킹index_min으로 바꾸어준다. (기존 마킹 소멸)  
